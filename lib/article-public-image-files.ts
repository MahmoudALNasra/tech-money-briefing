...
const ARTICLE_MEDIA_ROOT = resolve(process.cwd(), "public/media/articles");
...
export async function importArticleImageToPublic(input: {
  ...
  const directory = resolve(ARTICLE_MEDIA_ROOT, String(year), month);
  ...
  return `/media/articles/${year}/${month}/${filename}`;
}
